...
cached.path.1 <- "cached_data/7_process_summarize_optics/rds"
cached.path.2 <- "cached_data/mapping"
cached.figures <- "cached_figures"

##############
#  Lab Data  #
##############
setwd("M:/QW Monitoring Team/GLPF/Data/R/cacheMerge")
df <- readRDS(file.path(cached.path.1, "summary_noQA.rds"))
df <- filter(df, State == "WI")

######################
#  Site information  #
######################
ssloc <- readRDS(file.path(cached.path.2,"siteWI.rds"))
basinspecs <- readRDS(file.path(cached.path.2,"basinSpecs.rds"))

#######################################
#  Prepping Static Base Data for Map  #  My mapping stuff
#######################################
...
##############
#  Map Stuff #  Laura's mapping stuff 
##############
hydro_line <- readRDS(file.path(cached.path.2, "hydro_line.rds"))
...
#Simplify data
dfs <- df[,c('SiteID','USGSFieldID','pdate','eventNum',bacteria)]

#Create stacked dataset
dfs.s<-melt(dfs,id=c("SiteID","USGSFieldID","pdate","eventNum"))
...
dfs.s.sub <- dfs.s %>%
  separate(USGSFieldID,c("Site","sampleNum"),"-") %>%
