...
lambdaType <- c("lambda.1se")
filenm <- "Lachno3Lasso1se.pdf"
pdf(filenm)
modelCoefList <- list()
for(i in 1:length(lambdaType)){
  
  subdf <- df[which(!is.na(df[,"ent"])),]
  IVs <- names(df)[begin:end]
  ...
  if(length(IVs) > 0){
    ...
    summary(msurvStd)
    mStep <- stepAIC(msurvStd,trace = 0)
    summary(mStep)
    coefStep <- coef(mStep)
  ...
  for (j in 1:length(plotEvents)){
    ...
    mtext(paste0(response[1],"; Survival; ",plotEvents[j]),line=1)
    mtext(paste(names(coefStep)[-1],collapse=' + '),cex=0.7)
    mtext(lambdaType[i],line=2,font=2)
    abline(h=4,v=10000,col="blue",lty=2)
  }
